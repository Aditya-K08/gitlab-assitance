 ...
 */ class GeminiEmbedding extends BaseEmbedding {
    ...
    async embedWithRetry(args) {
        const MAX_TRIES = 20;
        const DELAY_MS = 5000;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const isRateLimitError = (err)=>{
            if (!err) return false;
            return err.status === 429 || err.message.indexOf('"status":"RESOURCE_EXHAUSTED"') !== -1;
        };
        ...
        for(let attempt = 1; attempt <= MAX_TRIES; attempt++){
            try {
                ...
                if (isRateLimitError(err) && attempt < MAX_TRIES) {
                    ...
                    console.debug(`Gemini Embeddings rate limit error encountered. Retrying attempt ${attempt}...`);
                    continue;
                }
                throw err;
            }
